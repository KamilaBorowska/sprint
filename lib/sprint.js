(function() {
  ...
  global.sprint = function() {
    ...
    return string.replace(format, function() {
      ...
      if (vector) {
        character = vector[0] === '*' ? vector.length > 2 ? values[parseInt(vector.slice(1)) - 1] : values[++i] : '.';
      }
      length = length[0] === '*' ? length.length === 1 ? values[++i] : values[parseInt(length.slice(1)) - 1] : !length ? 0 : length;
      precision = precision && precision[0] === '*' ? precision.length === 1 ? values[++i] : values[parseInt(precision.slice(1)) - 1] : !precision ? (defaultPrecision = true, 6) : precision;
