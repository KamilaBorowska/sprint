...
(function() {
  ...
  format = /%(?:%|(\d+[$])?((?:[+\x20\-#0])*)((?:[*](?:\d+[$])?)?v)?(\d*|[*](?:\d+[$])?)(?:[.](\d+|[*](?:\d+[$])?))?(hh?|ll?|[Lzjtq]|I(?:32|64)?)?([diuDUfFeEgGxXoOscpnbBr]))/g;
  ...
  sprint = function() {
    var arrayObjects, dearrayified, i, padString, string, toString, values, _ref;
    ...
    return ("" + string).replace(format, function(string, argument, flags, vector, length, precision, intSize, type) {
      var abs, alignCharacter, arg, args, character, defaultPrecision, leftPad, letter, padInteger, prefix, result, special, toExponential;
      if (intSize == null) {
        intSize = 'L';
      }
      if (string === '%%') {
      ...
      abs = function(number, signed) {
        var bits, entry, highValue;
        if (signed == null) {
          signed = false;
        }
        if (intSize === 'L' && (number >= 0 || signed)) {
        ...
        if (signed && number >= Math.pow(2, bits - 1)) {
          ...
          argument = (function() {
            ...
            switch (type) {
              ...
              case 'G':
                ...
                if (+arg === 0 || (0.0001 <= (_ref1 = Math.abs(arg)) && _ref1 < Math.pow(10, precision))) {
